...
@section('content_header')
    <h1>Jumlah Layanan Tindakan Operasi</h1>
@stop

@section('content')
    <div class="row">
        <div class="col-md-12">
            <x-adminlte-card title="Input Jumlah Tindakan Operasi" theme="dark" theme-mode="outline" collapsible="collapsed">
                <div class="row">
                    <div class="col-md-12">
                        @php
        ...
        <div class="col-md-12">
            <x-adminlte-card title="Data Jumlah Layanan Laboratorium (parameter)" theme="dark" theme-mode="outline">
                ...
                @endphp
                <x-adminlte-datatable id="tableOperasi" :heads="$head" head-theme="dark" :config="$config" striped hoverable bordered compressed>
                    @forelse($data as $data)
                        <tr @if($data->response == 'MSG20003') class="bg-success" @endif>
                            <td>{{ $data->tgl_transaksi }}</td>
                            <td>{{ $data->klasifikasi_operasi }}</td>
                            <td>{{ $data->jumlah }}</td>
                            <td>{{ $data->response }}</td>
                            <td>{{ $data->send_at }}</td>
                            <td>{{ $data->updated_at }}</td>
                            <td>
                                <x-adminlte-button label="Kirim Ulang" onclick="kirimUlang('{{$data->tgl_transaksi}}','{{$data->klasifikasi_operasi}}','{{$data->jumlah}}')" class="btn-sm" icon="fas fa-lg fa-save"  />
                            </td>
                        </tr>
                    @empty
                        <tr>
...
@section('js')
    ...
    <script>
        ...
        function kirimUlang(tgl,klasifikasi,jml) {
            let data = {
                _token:$('meta[name="csrf-token"]').attr('content'),
                tgl_transaksi:tgl,
                klasifikasi_operasi:klasifikasi,
